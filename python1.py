...
def one(input1, input2):
	if len(input1) > len(input2): #if first input is greater than second
		return input1             #output first input
	elif len(input1) < len(input2): #if first input is less than second 
		return input2				#output second input
	else:
		return input1 + " " + input2 #outputs solution in correct format
...
#def two(input):
#	upper_case = input.upper() #ignores case
#	start='BERT'
#	end='BERT'
#	if upper_case[upper_case.find(start)+len(start):upper_case.rfind(end)]:
#		return input[start:end]	
#print(two("BERTIWANTBERT"))
...
def three(arg1):
	if arg1 % 3 == 0 and arg1 % 5 ==0: #if input is divisible by both 5 and 3 print "fizzbuzz"
		return "fizzbuzz"
	elif arg1 % 3 == 0: #if input is only divisible by 3 print "fizz"
		return "fizz"
	elif arg1 % 5 == 0: #if input is only divisible by 5 print "buzz"
		return "buzz"
	else:
		return "null" #otherwise print "null"
...
#def five(owner,nameoffile,encryption,fileSize):
	#encoded_char = []
...
def six(input):
    if "cei" in input:  #Boolean statements checking if input follows mnemomic
        return True
    elif "cie" in input:
        return False
    elif "ie" in input:
...
def seven(input):
	vowel_no = 0 #sets initial value to zero
	vowel_number = list(input.lower()) #sets case for input value as lower case
	for vowels in range(len(vowel_number)): #for loop defining values within input
		if vowel_number[vowels] == "a" or vowel_number[vowels] == "e" or vowel_number[vowels] == "i" or vowel_number[vowels] == "o" or vowel_number[vowels] == "u": #defines vowels
			vowel_no += 1 #sums vowels present in input
	return vowel_no
...
def eight(input):
	import math #imports factorial function
	if input <1 or input >10: #input must be between 1 and 10
		print("Try another input between 1 and 10")
	else:
		if input == 0:
			return 1
		return input * math.factorial(input - 1) #performs factorial on input
...
def nine(inputString, char):
	char_element = -1 #returns -1 if char does not occur
	format_space = inputString.replace (" ", "") #format to ignore white space
	format_space
	print(format_space)
	find_char = list(format_space)     #finds character in list
	for characters in range(len(find_char)): #looks for character in string
		if find_char[characters] == char:
			char_element = characters + 1 #adds 1 to each character value
			break
	return char_element
...
def ten(string, int, char):
	string_len = len(string) #defines length of string
	if string_len < int: #if string length is less than that of integer print false
		return False
	elif char in string.lower() [int-1]: #if string length is greater than that of integer print true
		return True
	else:
		return False
